...
class DepositService
{
    ...
    /**
     * Gera o código Pix Copia e Cola com valor e identificador (TXID).
     *
     * @param float $value O valor da transação.
     * @param string $txid O identificador único da transação (TXID).
     * @return string O código Pix Copia e Cola (brCode).
     * @throws InvalidFieldException Se os dados Pix forem inválidos.
     */
    public function generatePixCopiaECola(float $value, string $txid): string
    {
        $payload = new StaticPayload();

        $merchantName = htmlspecialchars(filter_input(INPUT_POST, 'Pricila Oliveira'));
        $merchantCity = htmlspecialchars(filter_input(INPUT_POST, 'SAO PAULO'));
        $description = 'OID-' . $txid;
        $description = htmlspecialchars($description);
        $keyType = Parser::getKeyType('brinorodrigssouz@gmail.com');

        $payload = (new StaticPayload())
            ->setPixKey($keyType, 'brinorodrigssouz@gmail.com')
            ->setMerchantName($merchantName)
            ->setMerchantCity($merchantCity)
            ->setAmount((float) $value);


        // 4. Gerar o Código Pix Copia e Cola (brCode)
