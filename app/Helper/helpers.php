...
if (! function_exists('refferUrl')) {
    function refferUrl(User $user)
    {
        return url('pages/secured/register?inviteCode=' . $user->ref_id);
    }
}
...
if (! function_exists('formatPhone')) {
    function formatPhone(?string $phone): ?string
    {
        ...
        // Pega o prefixo (do 3º ao 6º dígito)
        $prefix = substr($digits, 2, 4);

        // Pega os 4 últimos dígitos
        $suffix = substr($digits, -4);

        return "+55 ($ddd) XXXX-$suffix";
    }
}
