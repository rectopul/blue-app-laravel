...
@section('content')
    <div x-data="withdrawApp()" class="pb-32">
        ...
        <div class="px-5 -mt-6 space-y-6">
            ...
            <div class="bg-white rounded-[32px] p-6 shadow-xl shadow-slate-200/50 border border-slate-50">
                ...
                <template x-if="hasPix && !editingPix">
                    <div class="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                        <div class="w-12 h-12 rounded-full bg-white flex items-center justify-center text-[#2C95EF] shadow-sm">
                        ...
                        <div class="flex-1 overflow-hidden">
                            ...
                            <span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-[#EAF4FF] text-[#2C95EF] uppercase mt-1 inline-block" x-text="pixType"></span>
                        </div>
                    </div>
                </template>
                ...
                <div x-show="!hasPix || editingPix" x-cloak class="space-y-4">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 ml-1">Nome Completo do Titular</label>
                        <input type="text" x-model="pixForm.realname" class="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm font-medium focus:ring-2 focus:ring-[#2C95EF] transition-all" placeholder="Como consta no banco">
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 ml-1">Tipo de Chave</label>
                        <select x-model="pixForm.pix_type" class="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm font-medium focus:ring-2 focus:ring-[#2C95EF] transition-all">
                    ...
                    <div>
                        ...
                        <input type="text" x-model="pixForm.pix_key" class="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm font-medium focus:ring-2 focus:ring-[#2C95EF] transition-all" :placeholder="pixForm.pix_type === 'CPF' ? '000.000.000-00' : (pixForm.pix_type === 'PHONE' ? '(00) 00000-0000' : 'seu@email.com')">
                    </div>
                    <button @click="savePix()" :disabled="savingPix" class="w-full bg-slate-900 text-white font-bold py-4 rounded-2xl active:scale-[0.98] transition-all disabled:opacity-50">
            ...
            {{-- Withdraw Form --}}
            <div class="bg-white rounded-[32px] p-6 shadow-xl shadow-slate-200/50 border border-slate-50" x-show="hasPix && !editingPix">
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 text-center">Quanto deseja sacar?</label>
                ...
                <div class="grid grid-cols-4 gap-2 mb-8">
                    @foreach ([50, 100, 200, 500] as $val)
                        <button @click="withdrawAmount = {{ $val }}"
                            :class="withdrawAmount == {{ $val }} ? 'bg-[#2C95EF] text-white' : 'bg-slate-50 text-slate-600'"
                ...
                <div class="bg-blue-50 rounded-2xl p-4 mb-8 space-y-2">
                    <div class="flex justify-between text-[10px] font-bold">
                        ...
                        <span class="text-slate-600">R$ <span x-text="formatMoney(withdrawAmount * {{ (float)setting('withdraw_charge') }} / 100)"></span></span>
                    </div>
                    <div class="flex justify-between text-xs font-black pt-2 border-t border-blue-100">
                        ...
                        <span class="text-slate-900 font-black text-sm">R$ <span x-text="formatMoney(withdrawAmount - (withdrawAmount * {{ (float)setting('withdraw_charge') }} / 100))"></span></span>
                    </div>
                </div>

                <button @click="requestWithdraw()" :disabled="requesting || withdrawAmount < {{ (float)setting('minimum_withdraw') }}"
                ...
                <p class="mt-4 text-[9px] text-center text-slate-400 font-bold uppercase tracking-widest">
                    Mínimo: R$ {{ number_format(setting('minimum_withdraw'), 2, ',', '.') }} | Máximo: R$ {{ number_format(setting('maximum_withdraw'), 2, ',', '.') }}
                </p>
            </div>
        </div>
    </div>
