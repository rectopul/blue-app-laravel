    ...
    <div x-data="depositApp()" class="pb-32">
        ...
        <div class="px-5 mt-6">
            <div class="bg-white rounded-[32px] p-6 shadow-xl shadow-slate-200/50 border border-slate-50">
                ...
                <div class="grid grid-cols-3 gap-3 mb-8">
                    @foreach ([50, 100, 200, 500, 1000, 2000] as $val)
                        <button @click="amount = {{ $val }}"
                            :class="amount == {{ $val }} ?
                                'bg-[#2C95EF] text-white shadow-lg shadow-blue-200 ring-transparent' :
                ...
                <p class="mt-4 text-[10px] text-center text-slate-400 font-medium">Depósito mínimo: R$
                    {{ number_format($minDeposit, 2, ',', '.') }}</p>
            </div>
            ...
            <div class="mt-8 space-y-4">
                ...
                <div class="flex items-center gap-4 bg-white/50 p-4 rounded-[22px] border border-slate-100">
                    ...
                    <p class="text-xs text-slate-500 leading-relaxed">Gere o código PIX acima informando o valor desejado.
                    </p>
                </div>
                ...
                <div class="flex items-center gap-4 bg-white/50 p-4 rounded-[22px] border border-slate-100">
                    ...
                    <p class="text-xs text-slate-500 leading-relaxed">O saldo será creditado automaticamente após a
                        compensação.</p>
                </div>
            </div>
        </div>
        ...
        <div x-show="pixModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                class="relative w-full max-w-sm bg-white rounded-[40px] overflow-hidden shadow-2xl animate-in zoom-in duration-300">
                ...
                <div class="p-8 bg-white relative">
                    <div class="flex flex-col items-center">
                        ...
                        <div class="text-center mb-8">
                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Valor do
                                Depósito</span>
                            <h2 class="text-3xl font-black text-slate-900">R$ <span x-text="formatMoney(amount)"></span>
                            </h2>

                            <div
                                class="mt-3 inline-flex items-center gap-2 bg-amber-50 px-3 py-1.5 rounded-full text-amber-600">
                        ...
                        <div class="w-full">
                            <p class="text-[10px] font-bold text-slate-400 uppercase mb-3 ml-1 tracking-widest">Código Copia
                                e Cola</p>
                            <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex items-center gap-3 group">
                                <div class="flex-1 overflow-hidden">
                                    <code class="text-[10px] text-slate-500 truncate block font-mono"
                                        x-text="pixCode"></code>
                                </div>
                                <button @click="copyCode()"
                                    class="bg-slate-900 text-white px-4 py-2.5 rounded-xl text-[10px] font-bold active:scale-95 transition-all"
                                    x-text="copyBtnText">
                                    COPIAR
                                </button>
                            </div>
                ...
                <button @click="closePixModal()"
                    class="w-full py-5 bg-slate-50 text-slate-400 font-bold text-[10px] uppercase tracking-widest hover:text-slate-600 border-t border-slate-100 transition-colors">
                    Fechar Janela
                </button>
            </div>
        </div>
