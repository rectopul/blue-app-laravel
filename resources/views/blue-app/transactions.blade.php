    ...
    <div class="pb-32">
        ...
        <div class="relative overflow-hidden rounded-b-[34px] bg-gradient-to-b from-[#CFE7FF] to-[#EEF4F9] px-5 pt-8 pb-10">
            <div class="flex items-center justify-between">
                <a href="{{ route('profile') }}" class="grid h-11 w-11 place-items-center rounded-2xl bg-white shadow-sm ring-1 ring-black/5 active:scale-90 transition-all">
        ...
        <div class="px-5 -mt-6">
            <div class="space-y-3">
                @forelse($ledgers as $ledger)
                    <div class="bg-white rounded-[24px] p-5 shadow-sm border border-slate-100 flex items-center justify-between group active:scale-[0.98] transition-all">
                        ...
                        <div class="text-right">
                            <p class="text-base font-black tracking-tight {{ $ledger->credit > 0 ? 'text-emerald-500' : 'text-red-500' }}">
                                {{ $ledger->credit > 0 ? '+' : '-' }} R$ {{ number_format($ledger->credit > 0 ? $ledger->credit : $ledger->debit, 2, ',', '.') }}
                            </p>
                            <span class="text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider
                ...
                @empty
                    <div class="py-20 text-center bg-white rounded-[40px] border border-dashed border-slate-200">
                        ...
                        <p class="text-sm font-bold text-slate-400 uppercase tracking-widest">Nenhuma transação encontrada</p>
                    </div>
                @endforelse
            </div>
